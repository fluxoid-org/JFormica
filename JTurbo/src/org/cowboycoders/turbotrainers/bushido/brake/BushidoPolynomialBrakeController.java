 ...
 *
 * A brake resistance controller which maps a speed to a brake resistance. 
 * 
 * The mapping is implemented using a polynomial fit based on observations 
 * of the head unit behavior. 
 *
 */
public class BushidoPolynomialBrakeController implements TurboTrainerDataListener {
	
	//Period at which the virtual speed will be updated (ms)
	private static final int  POWER_MODEL_UPDATE_PERIOD_MS = 100;
	
	//To stop furious pedaling at the start we choose a reasonable initial resistance
	...
	public BushidoPolynomialBrakeController(BushidoData bushidoModel) {
	 ...
	 * 
	 * The virtual speed is calculated using a power model taking
	 * into account rider and bike weight (total weight), the current
	 * slope, etc.
	 * 
	...
	public double getBrakeResistanceFromPolynomialFit() {
		...
		double brakeResistance;
		double slope = bushidoDataModel.getSlope();//Look in headunit and make slope model
		double totalWeight = bushidoDataModel.getTotalWeight();
		double speed = bushidoDataModel.getSpeed();
		
		// Use a polynomial fit to data ripped from the head unit to estimate brake resistance from
		// total weight, speed and slope
		
		brakeResistance = slope*POLY_A 
				+ totalWeight*POLY_B
				+ totalWeight*slope*POLY_C
				+ speed*POLY_D 
				+ Math.pow(speed, 2)*POLY_E + POLY_F 
				+ slope*speed*POLY_G 
				+ totalWeight*speed*POLY_H 
				+ slope*totalWeight*speed*POLY_I;
